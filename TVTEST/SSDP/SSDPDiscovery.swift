...
public class SSDPDiscovery: NSObject {
    ...
    open func stopAllDiscovery() {
        // Close all sessions
        activeSessions.forEach { $0.forceClose() }
    ...
    fileprivate var activeSessions: [SSDPDiscoverySession] = []
...
extension SSDPDiscovery {
    ...
    internal func closeSession(session: SSDPDiscoverySession) {
        ...
        self.activeSessions = activeSessions.filter { $0 !== session as SSDPDiscoverySession? }
